...
Some centralized communication systems are said to be *non-repudiable* because their users purposely want to hold their counterparties legally accountable. Citium does not cater for that purpose. In fact, Citium offers the complete opposite: **deniability**, which is the last line of defense against **forced disclosure** or [_mandatory key disclosure_](https://en.wikipedia.org/wiki/Key_disclosure_law). When two users (e.g., Joe and Jane) decide to communicate through Citium with each other, they have to become each other's authenticated users ("Contacts") in Citium first. But as ironic as it may sound, no one, not even themselves, can prove their authenticated Contact relationship even during the course of their communication. This is achieved by two innovative and unique InfoSec mechanisms: **sockpuppetry**. [Sockpuppets](https://en.wikipedia.org/wiki/Sockpuppet_(Internet)) are decoys. Citium's sockpuppet mechanism dictates that no user can send message directly to another user but indirectly through the sea of sockpuppeted user account in Citium. From an aerial perspective, all users look like sockpuppets. Everyone looks like a decoy. No one, not even the communicating counterparty (Joe or Jane), can differentiate directly-controlled user accounts from the sockpuppeted ones except his/her own.
