...
西蒂姆（Citium）作為一個免費的、開源代碼的、完全去中心化的、[無需准入許可的區塊鏈系統](https://en.wikipedia.org/wiki/Blockchain#Permissionless) 的好處是 **抗审查**。沒人可被阻止在西蒂姆（Citium）上架設節點。節點的運營商，例如即時聊天工具提供商（IMSP），可以向通過其西蒂姆（Citium）節點訪問的用戶宣傳自己的資料（例如，商業廣告內容）。每個節點都可以向西蒂姆（Citium）多點傳送/檢索數據（文本、圖像、視頻和實時語音）。任何兩個用戶（例如Alice和Bob）都可以隨時通過任何西蒂姆（Citium）的IMSP進行安全和可推諉的通訊而無需徵詢任何其他人的許可。當然，任何一個西蒂姆（Citium）的用戶或節點運營商都有權不為某些用戶或節點提供服務，完全取決於每個參與者的決定，自由自決。無論從哪個網絡層面查看西蒂姆（Citium）的數據，所有數據看起來都是雷同的。沒有第三者（尤其是機器智能）可以區分真實數據與偽造數據，因為每個人都可以偽造甚至編輯其他人的數據。簡而言之，除非另有證明，否則所有數據均假定為未知來源且不可信。

The main benefit of Citium being a free, open-source, fully decentralized, [permissionless blockchain](https://en.wikipedia.org/wiki/Blockchain#Permissionless) is __censorship-resistance__. No one can be banned from running nodes. Operators of nodes (e.g., Instant Messenger System Provider (IMSP)) may advertise their own material (e.g., commercial content) to those who access Citium through their nodes. Every node can multicast/retrieve data (text, image, video and real-time voice) to/from Citium. Any two users (e.g., Alice & Bob) who decide to communicate securely and deniably may hop on any IMSP of Citium at any time without the need to ask anyone else for permission. And of course, any Citium user or node owner is entitled to not serve certain users or nodes. It all depends on the decision of each participant (self-determination). No matter from which network communication layer that one looks at Citium, all data looks the similar. No third party, especially machine intelligence, can differentiate genuine from forged data because everyone can forge and even edit someone else's data. In short, all data are assumed unknown origin and untrustworthy until proven otherwise.
...
當兩個用戶（例如Alice和Bob）決定通過西蒂姆（Citium）通信，他們必須首先成為西蒂姆（Citium）中彼此認證的用戶（“聯絡人”）。添加聯絡人這個認證行為也是唯一次時機他們倆（Alice和Bob）可以彼此確認身份。因為在此之後，即使在他們倆通信過程中，任何人都無法無可辯駁地證明他們兩個的聯絡人關係。
