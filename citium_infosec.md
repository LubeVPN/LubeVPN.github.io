...
## 驗證性 | Authentication ✓
In Citium, sockpuppetry dictates that __anyone can pretend to be someone else__. User account nickname is non-exclusive, too! No user knows for sure which account belongs to whom from an aerial perspective, except their own. Despite what has just been said, the traditional sense of user authentication (i.e., irrefutably identifying a user) is still there because authentication in the Citium universe is no longer bounded by user account alone but by every cryptographically signed message. Two users (e.g., Joe & Jane) must be joined cryptographically or authenticated in a trusted relationship, i.e., "Contacts", before they can communicate. Once authenticated, message sent between two Contacts (e.g., Joe & Jane) cannot be spoofed by any third party. That is, Jane can always correctly identify the cryptographically bounded message sent from Joe whom she has authenticated from the outset in spite of many other users pretending to be Joe; and Joe can always be certain that only the one true Jane is authorized to read the message that he send in spite of many other users pretending to be Jane trying to access the message.

## 機密性 | Confidentiality ✓
傳統IMSP是基於中心化身份驗證和授權機制去確保信息安全的，天然使然無可避免地需要擔受[數據洩露](https://en.wikipedia.org/wiki/Data_breach)的風險。這是西蒂姆（Citium）的機制完全解決了的問題。（[請參閱專用頁面](../ fallible_providers)）西蒂姆（Citium）使用獨特的身份驗證機制來更好地檢查和平衡用戶與IMSP之間的關係：**用戶身份驗證信息完全由用戶自己生成，絕不假手于他人** 未經授權的第三方均無法讀取託管在Joe的IMSP服務器上的適配給他的聯絡人的訊息（例如，發訊者Joe留給Jane的訊息，除了Jane之外無人能讀），因為Citium使用了 __無差別網樹多點傳送__（IMTM）__哈希傳遞網路__（HDN）機制。
<br>
Traditional IMSP based on centralized authentication and authorization regime is inherently and inevitably susceptible to [data breach](https://en.wikipedia.org/wiki/Data_breach). This is a paint point that Citium's regime are made to resolve. ([Refer to the dedicated page that talks about it.](../fallible_providers)) Citium use a unique authentication mechanism for better checks and balances between users and IMSP: **A user authentication info is entirely generated by the user but no one else.** No unauthorized third party can read an appropriate message (e.g., sent from Joe to Jane), which is hosted on Joe's IMSP server, because Citium use a **indiscriminate mesh-tree multicast** (IMTM) **hash delivery network** (HDN) mechanism.

### IMTM HDN
IMTM-HDN 意味著 __消息是被算法劃分成多個散列密鑰__，這些散列密鑰又通過網狀樹多點傳送、不加選擇地分佈到盡可能多的節點上。存放加密的哈希密鑰的西蒂姆（Citium）節點充當接收者（Jane）的「哈希傳遞網絡」（HDN）。為了使接訊者（Jane）解密來自發訊者（Joe）的消息，Jane必須獲取指定的私鑰來解密消息。 Jane必須通過 __無差別網樹多點傳送__（IMTM）來盡可能多的節點作請求，直到她收集形成指定的專用密鑰所需的所有散列的哈希密鑰為止。__只有接訊者（Jane）才能將所有哈希密鑰重新統一併起來才能生成有效的私鑰，成功解鎖Joe留給她的加密的信息__。
<br>
**Indiscriminate mesh-tree multicast** (IMTM) **hash delivery network** (HDN) means that __the message is algorithmically split into multiple hash keys__, which in turn are distributed indiscriminately to as many nodes as possible by mesh-tree multicasting. The Citium nodes that hold the encrypted hash keys act as **hash delivery network** (HDN) for the receiver (Jane). In order for the receiver (Jane) to decrypt the message from the sender (Joe), Jane has to obtain the designated private key to decrypt the message. Jane has to make request to as many nodes as she can through **indiscriminate mesh-tree multicast** (IMTM) until she collects all the hash keys necessary to form the designated private key. _Only the receiver (Jane) can reunite and decrypt all hash keys back to the valid private key to unlock the encrypted message_.
